...
def test_different_fingerprints_both_investigated(client):
    ...
    with patch("forager.agent.investigate", return_value=inv) as mock_inv:
        client.post("/webhook/alertmanager", json={"alerts": [{
            "status": "firing", "fingerprint": "fp_aaa",
            "labels": {"alertname": "Alert", "service": "svc"}, "annotations": {},
        }]})
        client.post("/webhook/alertmanager", json={"alerts": [{
            "status": "firing", "fingerprint": "fp_bbb",
            "labels": {"alertname": "Alert", "service": "svc"}, "annotations": {},
        }]})
...
def test_dashboard_shows_investigation(client):
    ...
    with patch("forager.agent.investigate", return_value=inv):
        client.post("/webhook/alertmanager", json={"alerts": [{
            "status": "firing", "fingerprint": "dash_fp",
            "labels": {"alertname": "DashAlert", "service": "api"}, "annotations": {},
        }]})
    resp = client.get("/dashboard")
    assert resp.status_code == 200
